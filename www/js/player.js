...
Player.main = function() {
	Player.player = window.document.getElementById("player");
	Player.list = window.document.getElementById("playlist");
	Player.initControls();
	Player.nav = window.document.getElementsByClassName("demo-navigation")[0];
	Player.clear(Player.nav);
	var hash;
	if(window.location.hash == null) hash = ""; else {
		var _this = window.location.hash;
		hash = HxOverrides.substr(_this,1,null);
	}
	var init = false;
	var _g = 0;
	var _g1 = Player.playlists;
	while(_g < _g1.length) {
		var playlist = [_g1[_g]];
		++_g;
		var a;
		...
		a.innerHTML = "" + playlist[0].host + " - " + playlist[0].name;
		a.href = "";
		a.setAttribute("data-playlist",playlist[0].name);
		a.onclick = (function(playlist) {
			return function(e) {
				e.preventDefault();
				if(Player.currentPlaylist != playlist[0]) Player.loadPlaylist(playlist[0]);
			};
		})(playlist);
		Player.nav.appendChild(a);
		if(hash.toLowerCase() == playlist[0].name.toLowerCase()) {
			Player.loadPlaylist(playlist[0]);
			init = true;
		}
	}
	if(!init) Player.loadPlaylist(Player.playlists[0]);
};
Player.loadPlaylist = function(playlist) {
	if(!Lambda.exists(Player.playlists,function(p) {
		return p == playlist;
	})) {
		console.log("Playlist " + Std.string(playlist) + " doesn't exist");
		return;
	}
	window.document.getElementsByClassName("demo-list")[0].style.display = "none";
	window.document.getElementById("loading").style.display = "block";
	Player.currentPlaylist = playlist;
	var _g = 0;
	var _g1 = Player.nav.children;
	while(_g < _g1.length) {
		var child = _g1[_g];
		++_g;
		child.classList.remove("active");
		var data = child.getAttribute("data-playlist");
		if(Player.currentPlaylist.name == data) child.classList.add("active");
	}
	Player.playedTracks = [];
	Player.player.pause();
	Player.player.src = null;
	var http = new haxe_Http(playlist.src);
	http.async = true;
	http.onData = function(str) {
		var tracks = [];
		try {
			var xml = new haxe_xml_Fast(Xml.parse(str).firstElement()).node.resolve("trackList");
			var _g2 = xml.nodes.resolve("track").iterator();
			while(_g2.head != null) {
				var track;
				track = (function($this) {
					var $r;
		...
		Player.player.onended = (function(f,a1) {
			return function() {
				f(a1);
			};
		})(Player.playRandomTrack,tracks);
		window.document.getElementById("skip_next").onclick = (function(f1,a11) {
			return function() {
				f1(a11);
			};
		})(Player.playRandomTrack,tracks);
		Player.clear(Player.list);
		var _g3 = 0;
		while(_g3 < tracks.length) {
			var track1 = tracks[_g3];
			++_g3;
			Player.list.appendChild(Player.createLI(track1));
		}
		window.document.getElementsByClassName("demo-list")[0].style.display = "block";
		window.document.getElementById("loading").style.display = "none";
		Player.playRandomTrack(tracks);
	};
	http.request();
};
Player.createLI = function(track) {
	var li;
	...
	li.onclick = (function(f,a1,a2) {
		return function() {
			f(a1,a2);
		};
	})(Player.playTrack,track,false);
	track.li = li;
...
Player.initControls = function() {
	...
	var percentage = window.document.getElementById("percentage");
	play_button.onclick = function() {
		if(Player.player.paused) Player.player.play(); else Player.player.pause();
	};
	window.document.getElementById("volume_up").onclick = function() {
		Player.player.volume += 0.1;
	};
	window.document.getElementById("volume_down").onclick = function() {
		Player.player.volume -= 0.1;
	};
	var seek = function() {
		var playing = !Player.player.paused;
		if(playing) Player.player.pause();
		Player.player.fastSeek(Player.player.duration * percentage.valueAsNumber / 100);
		if(playing) Player.player.play();
	};
	percentage.onclick = seek;
	percentage.onchange = seek;
	Player.player.onpause = function() {
		play_button.firstElementChild.innerHTML = "play_arrow";
		window.document.getElementById("info").innerHTML = "" + Player.currentTrack.author + " - " + Player.currentTrack.title;
	};
	Player.player.onplay = function() {
		play_button.firstElementChild.innerHTML = "pause";
		window.document.getElementById("info").innerHTML = "" + Player.currentTrack.author + " - " + Player.currentTrack.title;
	};
	Player.player.ontimeupdate = function() {
		window.document.getElementById("time").innerHTML = Player.formatTime(Player.player.currentTime,Player.player.duration);
		percentage.MaterialSlider.change(isNaN(Player.player.duration)?0:Player.player.currentTime / Player.player.duration * 100);
	};
};
Player.playTrack = function(track,scroll) {
	if(scroll == null) scroll = true;
	if(Player.currentTrack != null) {
		Player.currentTrack.li.classList.remove("playing");
		Player.playedTracks.push(Player.currentTrack);
	}
	track.li.classList.add("playing");
	if(scroll) track.li.scrollIntoView({ block : "start", behavior : "smooth"});
	Player.currentTrack = track;
	Player.player.src = track.file;
	Player.player.play();
};
Player.clear = function(e) {
	while(e.firstChild != null) e.removeChild(e.firstChild);
};
Player.formatTime = function(current,end) {
	return "" + (function($this) {
		...
		var $r;
		var t;
		if(end == null || isNaN(end)) t = 0; else t = end;
		var min1 = Math.floor(t / 60);
		var sec1 = Math.floor(t - min1 * 60);
		$r = "" + (min1 < 10?"0":"") + min1 + ":" + (sec1 < 10?"0":"") + sec1;
		return $r;
	}(this));
};
Player.playRandomTrack = function(tracks) {
	var t;
	t = tracks != null && tracks.length > 0?tracks[Math.floor((tracks.length - 1 + 1) * Math.random())]:null;
	while(Player.currentTrack == t) t = tracks != null && tracks.length > 0?tracks[Math.floor((tracks.length - 1 + 1) * Math.random())]:null;
	Player.playTrack(t);
};
var Std = function() { };
Std.__name__ = true;
...
Array.__name__ = true;
var __map_reserved = {}
Player.playlists = [{ host : "Aersia", name : "VIP", src : "http://vip.aersia.net/roster.xml"},{ host : "Aersia", name : "Mellow", src : "http://vip.aersia.net/roster-mellow.xml"},{ host : "Aersia", name : "Source", src : "http://vip.aersia.net/roster-source.xml"},{ host : "Aersia", name : "Exiled", src : "http://vip.aersia.net/roster-exiled.xml"},{ host : "Aersia", name : "WAP", src : "http://wap.aersia.net/roster.xml"},{ host : "Aersia", name : "CPP", src : "http://cpp.aersia.net/roster.xml"}];
Player.playedTracks = [];
Xml.Element = 0;
Xml.PCData = 1;
Xml.CData = 2;
